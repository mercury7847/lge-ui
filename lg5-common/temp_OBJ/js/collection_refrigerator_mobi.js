$(function() {
    ...
    function scrollEventM() {
        ...
        if (scrollEM >= 30) {
            ...
            $(".only_mobile .ref-img img").attr("src", $(".only_mobile .ref-img img").attr("src").replace(/img-ref-big\.png$/, 'img-ref.png'));
        } else {
            $(".only_mobile .ref-img").removeClass("scroll");
            $(".only_mobile .ref-img img").attr("src", $(".only_mobile .ref-img img").attr("src").replace(/img-ref\.png$/, 'img-ref-big.png'));
        }
        ...
        if (scrollEM < refScM) {
            $(".only_mobile #reFscrollM").css("z-index", "4");
        }
        ...
        var divY2M = $(".only_mobile .info09W").offset().top;
        var fadeStartDiv2M = divY2M,
            fadeUntilDiv2M = divY2M + $(window).height();

        if (scrollEM >= fadeStartDiv2M) {
            if (opacity5M <= 0.64) {
                opacity5M = (scrollEM - fadeStartDiv2M) / (fadeUntilDiv2M - fadeStartDiv2M);
            } else {
                opacity5M = 0.64;
            }
            $(".only_mobile .info09W").css("background-attachment", "fixed");
        } else if (scrollEM > fadeUntilDiv2M) {
            opacity5M = 0.64;
        } else {
            $(".only_mobile .info09W").removeAttr("style");
            opacity5M = 0;
        }
        $(".only_mobile .infoMask").css({ 'opacity': opacity5M });
        ...
        if (scrollEM > fadeStartDiv4M && scrollEM < fadeUntilDiv4M) {
            if ($(".only_mobile .ct06-bg02 .ct-width").hasClass("wow2") == false) {
                $(".only_mobile .ct06-bg01 .ct-width").fadeOut(100).removeClass("wow2");
                $(".only_mobile .ct06-bg02 .ct-width").fadeIn(100).addClass("wow2");
            }
        } else if (scrollEM > fadeUntilDiv5M) {
            $(".only_mobile .ct06-bg02 .ct-width").fadeOut(100).removeClass("wow2");
        }
        ...
        var refrigM = $(".only_mobile .collc_info07").offset().top;
        var fadeStartDivM = refrigM,
            fadeUntilDivM = refrigM + $(window).height(),
            fadingDivM = $(".only_mobile .co07-refImg-move img");
        if (scrollEM >= fadeStartDivM && scrollEM < fadeUntilDivM) {
            if (scrollEM > fadeStartDivM && scrollEM <= fadeUntilDivM) {
                opacity4M = (scrollEM - fadeStartDivM) / (fadeUntilDivM - fadeStartDivM);
                $(".postion-new").css({
                    "position": "fixed"
                })
            }
        } else if (scrollEM > fadeUntilDivM) {
            opacity4M = 1;
        }
        fadingDivM.css({ 'opacity': opacity4M });

        if (scrollEM >= fadeUntilDivM) {
            $(".only_mobile .postion-new").css({
                "position": "absolute",
                "bottom": "0px",
                "top": "initial"
            });
        }
        if (scrollEM < fadeStartDivM) {
            $(".only_mobile .postion-new").css({
                "position": "absolute",
                "top": "0px",
                "bottom": "initial"
    ...
    function loadedM() {
        positionmM = document.getElementById('position');
        myScrollmM = new IScroll('.only_mobile #reFscrollM', {
            probeType: 3,
            scrollX: true,
            scrollY: false,
            mouseWheel: true,
        ...
        myScrollmM.on('scroll', updatePositionM);
        myScrollmM.on('scrollEnd', updatePositionM);
    ...
    function updatePositionM() {
        ...
        if (xM <= resWM) {
            ...
            $(".only_mobile #reFscrollM").css("z-index", "999");
        }
    }
