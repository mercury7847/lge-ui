$(function(){
	...
	function scrollEvent(){
		...
		if ( scrollE >= 0 && scrollE < 300 ) {
			var scrP = 1 - ( scrollE * 0.00008 );
			var tranlate = 'scale('+ scrP + ') translateX('+ - scrollE + 'px)';
			$(".only_pc .ref-img").css({ 
				transform: tranlate,
			});
			
			$(".only_pc .detail .ref-width").removeAttr("style");
			 
		 } else if ( scrollE >= 300 && scrollE < 1000 ) {
			var scrP = 1 - ( scrollE * 0.0008 );
			var tranlate = 'scale(0.8) translateY('+ - scrollE * 0.10 + 'px)';
			$(".only_pc .ref-img").css({ 
				transform: tranlate,
			});   
			$(".only_pc .detail .ref-width").css({
				"text-align": "center",
				"margin-left": "0px"
			});
		...
		if ( scrollE >= 300 ) {
			...
			$(".only_pc .ref-img img").attr("src",$(".only_pc .ref-img img").attr("src").replace(/img-washer-big\.png$/, 'img-washer.png'));
		} else { 
			$(".only_pc .ref-img").removeClass("scroll");
			$(".only_pc .ref-img img").attr("src",$(".only_pc .ref-img img").attr("src").replace(/img-washer\.png$/, 'img-washer-big.png'));
		}
		
		var fadeStart = 500, fadeUntil = 700, fadeOut = 1000, fadeEnd = 1200, fading = $(".only_pc .ref-bg");
		var fadeStart2 = 1200, fadeUntil2 = 1300, fadeOut2 = 2300, fadeEnd2 = 2400, fading2 = $("#reFscroll"), fading22 = $(".only_pc .collec-name");
		var opacity=0,opacity2=0,opacity3=0,opacity4=0,opacity5=0;
		...
		var newPosition = 0,newPosition2 = 0, opacitya=0;
		...
		var washScroll = $(".wash-info-list").offset().top;
		if ( scrollE >= washScroll ) {
			$(".only_pc .wash-info-list").css("opacity","1");
		} else if ( scrollE < washScroll ) {
			$(".only_pc .wash-info-list").css("opacity","0");
		}
