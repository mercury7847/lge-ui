(function() {
    ...
    var driverListTemplate = 
        ...
        '<li>' +
            ...
            '<p class="tit"><button type="button" class="btn-info" data-href="{{item.detailUrl}}" data-file-id="{{item.fileId}}" data-cseq="{{item.cSeq}}" data-os="{{item.os}}">{{#if item.os}}{{item.os}} {{/if}}{{item.title}}</button></p>' +
            '{{# } else { #}}' +
