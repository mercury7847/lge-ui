(function () {
  ...
  var autoCompleteItemTemplate =
    '<li><a href="#{{input}}">{{#raw text}}</a></li>';
  //카테고리아이템
  var categoryItemTemplate =
    '<li><a href="{{url}}" class="rounded"><span class="text">{{#raw text}}</span></a></li>';
  //제품미리보기
  ...
  var intergratedSearch = {
    ...
    setting: function () {
      ...
      //검색버튼
      self.$buttonSearch = self.$searchLayer.find(
    ...
    bindEvents: function () {
      ...
      self.$searchSimilar
        .off(".intergrated")
        .on("click.intergrated", "a", function (e) {
          e.preventDefault();
          self.searchItem($(this), true);
        });

      //인기검색어 클릭
      self.$popularKeywordList
        .off(".intergrated")
        .on("click.intergrated", "div.keyword-list ul li a", function (e) {
          e.preventDefault();
          self.searchItem($(this), true);
          ...
          arr.forEach(function (item, index) {
            $list_ul.append(
              vcui.template(autoCompleteItemTemplate, {
                input: item,
                text: vcui.string.replaceAll(item, searchedValue, replaceText),
              })
            );
          });
    ...
    requestSearch: function (value, isSaveRecentKeyword) {
      ...
      lgkorUI.requestAjaxData(ajaxUrl, { search: value }, function (result) {
        ...
        if (data.searchCount && data.category && data.category.length > 0) {
          self.$resultPreview
            .find("span.title-line em.count")
            .text("(" + vcui.number.addComma(data.searchCount) + ")");
        } else {
        ...
        //카테고리 리스트 갱신
        var arr =
          (data.category && data.category) instanceof Array
            ? data.category
            : [];
        ...
        if (arr.length > 0) {
          arr.forEach(function (item, index) {
            $list_ul.append(
              vcui.template(categoryItemTemplate, {
                url: item.url,
                text: vcui.string.replaceAll(
                  item.text,
                  searchedValue,
                  replaceText
                ),
    ...
    addRecentSearcheText: function (text) {
      ...
      lgkorUI.addCookieArrayValue(
        lgkorUI.INTERGRATED_SEARCH_VALUE,
        text,
        lgkorUI.MAX_SAVE_RECENT_KEYWORD
      );
      self.updateRecentSearchList();
    },
    ...
    updateBasicData: function () {
      ...
      lgkorUI.requestAjaxData(ajaxUrl, null, function (result) {
        ...
        if (arr.length > 0) {
          ...
          arr.forEach(function (item, index) {
            $list_ul.append(
              vcui.template(recommendItemTemplate, { text: item })
            );
          });
