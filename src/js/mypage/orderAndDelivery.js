...
//                             //isCashCheck = "현금결제";
//                         }else{
//                             isAllCancelDisable = true; // 변경 210907
//                             isCashCheck = "현금결제";
//                         }
//                     } else {
...
//                     }
                    if(data.itemCancelAbleYn == "Y"){
                     if((result.data.payment.paymentType == "41" || result.data.payment.paymentType == "42" || result.data.payment.paymentType == "0") && productList[0].itemStatus == "Ordered"){ //BTOCSITE-4124 210824 추가 41:계좌이체(현금/가상계좌) / 42:네이버페이 / 0:기타
                        ...
                        } else if(dataChk == true && $('#popup-cancel').hasClass('cash-chk') == false){
                            isAllCancelDisable = false;
                        } else {
                            isAllCancelDisable = false;
                        }
                ...
                //console.log("isAllCancelDisable %o %o %o",isAllCancelDisable,isCashCheck, dataChk );





                if (dataChk == "" && isAllCancelDisable == true && isCashCheck == ""){
                ...
                } else if(dataChk == false && isAllChecked == false && isAllCancelDisable == true && isCashCheck == "현금결제") { // 변경 210907
                    $('#popup-cancel').find('.ui_all_checker').prop('disabled', true);
                    $('#popup-cancel').find('input[name=chk-cancel]').prop('disabled', true);
                ...
                } else if(dataChk == true && isAllChecked == false && isAllCancelDisable == false && isCashCheck == "현금결제"){
            ...
            if((result.data.payment && dataChk == true && isAllCancelDisable == false && isCashCheck == "현금결제") || (result.data.payment && Object.keys(result.data.payment).length && result.data.payment.transType == METHOD_BANK && productList[0].itemStatus != "Ordered")){ //210826 추가 BTOCSITE-4124
                    ...
                    if(result.data.msg == "VC1001"){
                        if(flagChk == true){
                            lgkorUI.alert("", {
                                ...
                                isAllCancelDisable = false; // 210907 BTOCSITE-4124 추가
                                var dataID = $('#popup-cancel').data('dataId');
                                getPopOrderData(dataId, "ordercancel", opener); 
                                //popup.vcModal('close');//BTOCSITE-4124 210907 수정
                                //cancelSubmit();
                                }
                            });                       
                        }
                    }else {
                        // console.log("정상 취소 case");
                    }
